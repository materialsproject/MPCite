...
def test_builder_serialization(config_file_path: Path):
    ...
    new_materials_connection = MongoConnectionModel.parse_obj(new_d["materials_collection"])
    new_dois_collection_connection = MongoConnectionModel.parse_obj(new_d["dois_collection"])
    new_robocrys_collection_connection = MongoConnectionModel.parse_obj(new_d["robocrys_collection"])

    true_osti = OSTIModel.parse_obj(d["osti"])
    true_elsevier = ConnectionModel.parse_obj(new_d["elsevier"])
    true_materials_connection = MongoConnectionModel.parse_obj(new_d["materials_collection"])
    true_dois_collection_connection = MongoConnectionModel.parse_obj(new_d["dois_collection"])
    true_robocrys_collection_connection = MongoConnectionModel.parse_obj(new_d["robocrys_collection"])
    ...
    assert new_dois_collection_connection.dict() == true_dois_collection_connection.dict()
