...
from mpcite.models import OSTIDOIRecordModel, MongoConnectionModel, ConnectionModel

@pytest.fixture
def config_file_path():
    return Path(os.getcwd()) / "files" / "config-example.json"
...
def test_builder_serialization(config_file_path: Path):
    ...
    new_osti = OSTIDOIRecordModel.parse_obj(new_d["osti"])
    ...
    true_osti = OSTIDOIRecordModel.parse_obj(d["osti"])
