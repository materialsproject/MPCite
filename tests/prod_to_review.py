...
for record in query:
    ...
    print(
        f"On record #{count_materials_data}, next url is {query.next_url}, previous url is {query.previous_url}"
    )
    ...
    if record.title.startswith("Materials Data on"):
        ...
        new_record_osti_id = new_record_dict.pop(
            "osti_id"
        )  # now new_record_dict does not have the osti_id key.
        js = json.dumps(
            new_record_dict, default=str
        )  # datetime objects are not JSON serializable, so we use default=str to convert them to strings.
    ...
    if count_materials_data % rows_per_page == 0:
        ...
        print(
            f"Page {page_number} finished. Now at {count_materials_data} data entries. {count_materials_data - count_MaterialsDataOn} edge cases found."
        )
